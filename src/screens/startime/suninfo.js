...
boolGps =false;
    ...
    for( i=1; i<=untilDay; i++ ) {
        let timeDate  = SunCalc.getTimes(new Date(this.state.nowYear, this.state.nowMonth-1, i), har, lat);
        objData[i] = {};
        objData[i] = Object.assign(objData[i], {'suntime':timeDate});
    }
    console.log(objData, "objData");

    var outputKey = this.state.tableHead = ['date', 'sunrise','sunset'];
